...
<h1><?php echo $_SESSION['gmbt'];?>授权后台</h1>
 <h3 style="color:blue"><?php echo $_SESSION['name'];?> 欢迎登陆</h3> <br>
	 <button class="btn btn-info btn-block" onclick="window.location.href='gmstop.php'">禁发物品</button>
...
<form id="form1" name="form1" method="post" action="gmquery.php">
    ...
    <div class="form-group">
            <label for="v">GM禁发物品</label>
			<input type="text" placeholder="添加禁发物品ID"  class="form-control" id="item" name="item" value="" >
 <button type="submit" class="btn btn-info btn-block" name="sub" value="stop">添加GM禁发物品</button>	
         </div>
</form>
<div class="form-group">
