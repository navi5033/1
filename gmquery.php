...
$query1 = $mysql1->prepare("SELECT * FROM hero WHERE openid LIKE ? or name LIKE ? limit 1");
...
if ($_POST) {
	switch ($_POST['sub']) {
		case 'cz':
			...
			if (!empty($goodsid)) {
				$mysql1->set_charset('utf8');
				$query1 = $mysql1->prepare("insert into `pay` (`id`,`playerid`,`zoneid`,`goodsid`) values (?,?,?,?)");
				$query1->bind_param('ssss', $oid, $roleid, $zoneid, $goodsid);
				$query1->execute();
				file_put_contents($log, $date . "\t" . $zoneid . "\t" . $name . "\t" . $roleid . "\t" . $goodsid . "\t" . "success\t" . $user_IP . PHP_EOL, FILE_APPEND);
				echo '<script>alert(\'充值成功\');history.go(-1)</script>';
			} else {
				file_put_contents($log, $date . "\t" . $zoneid . "\t" . $name . "\t" . $roleid . "\t" . $goodsid . "\t" . "failed\t" . $user_IP . PHP_EOL, FILE_APPEND);
		...
		case 'mail':
			...
			if ($items != '') {
				$mysql1->set_charset('utf8');
				$query1 = $mysql1->prepare("INSERT INTO gmcmd (zoneid, cmd, param1, param2, param3, param4) values (?,'mail',?,?,?,?)");
				$query1->bind_param('sssss', $zoneid, $roleid, $items[0], $items[1], $items[2]);
				$query1->execute();
		...
		case 'allmail':
			...
			if ($itemtype == 0) {
				...
				if ($num < 1 || $num > 99999999) {
					echo "<script>alert('数量范围：1-99999999');history.go(-1)</script>";
					exit;
				}
				$itemstr .= $_POST['item'] .= '_';
			}
			...
			if ($items != '') {
				$mysql1->set_charset('utf8');
				$query1 = $mysql1->prepare("INSERT INTO gmcmd (zoneid, cmd, param1, param2, param3, param4) select zoneid,'mail',dbid ,?,?,? from players where level >?");
		...
		case 'yz1':
			...
			if ($roleid != '') {
				...
				while (!feof($file)) {
					...
					if (count($yzitem) == 3) {
						$mysql1->set_charset('utf8');
						$query1 = $mysql1->prepare("INSERT INTO gmcmd (zoneid, cmd, param1, param2, param3, param4) values (?,'mail','1',?,?,?)");
						$query1->bind_param('ssss', $zoneid, $roleid, $yzitem[0], $yzitem[1]);
						$query1->execute();
						usleep(30000);
		...
		case 'zhfh':
			...
			if ($roleid != '') {
				$mysql1->set_charset('utf8');
				$query1 = $mysql1->prepare("INSERT INTO gmcmd (zoneid, cmd, param1, param2) values (?,'Sealed',?,?)");
				$query1->bind_param('sss', $zoneid, $roleid, $time);
				$query1->execute();
		...
		case 'jy':
			...
			if ($roleid != '') {
				$mysql1->set_charset('utf8');
				$query1 = $mysql1->prepare("INSERT INTO gmcmd (zoneid, cmd, param1, param2) values (?,'Silent',?,?)");
				$query1->bind_param('sss', $zoneid, $roleid, $time);
				$query1->execute();
